...
// **write this function**
	...
	{
		uint8_t *target = (uint8_t *)(EDISK_ADDR_MIN + 512*sector);
		uint8_t *current = buff;
		for (int i = 0; i < SECTOR_SIZE; i++) 
		{
//			target = (uint8_t *)(EDISK_ADDR_MIN + 512*sector + i);
//			current = (uint8_t *)(&buff + i);
			current[i] = target[i];
		}
		return RES_OK;
...
// **write this function**
	...
	{
		//Flash_WriteArray(&buff, (EDISK_ADDR_MIN + 512*sector), SECTOR_SIZE);
		int err = Flash_WriteArray((uint32_t*)buff, (EDISK_ADDR_MIN + 512*sector), SECTOR_SIZE/4);
		if (err == SECTOR_SIZE/4)
		{
			return RES_OK;
		}
		else
...
// **write this function**
	for (uint32_t i = EDISK_ADDR_MIN; i<EDISK_ADDR_MAX; i=i+1024)
	{
