...
//Testing eDisk 
//int main(void){
int main_test_edisk(void){
	uint8_t testValue = 255;
	uint8_t readBuff[512];
  eDisk_Init(0);
  testbuildbuff("buf0 buf1 buf2");
  testValue = eDisk_Format();
  testValue = eDisk_WriteSector(Buff,0); // Put a breakpoint here an observe values from previous call
  testValue = eDisk_ReadSector(readBuff, 0);  // Put a breakpoint here and observe values from previous call
  return testValue;  // Put a breakpoint here and observe values from previous call
}

//Test eFile 
int main_test_eFile(void){
//int main(void){
   uint8_t fileA, fileB, fileC;
  //testbuildbuff("buf0 buf1 buf2");
  OS_File_Format();
  //MountDirectory(); // Put a breakpoint here an observe result from previous call.
                   //Memory window starting 0x00020000 (all 0xFF)
  fileA=OS_File_New(); // Put a breakpoint here and observe values from previous call.
                        // In Watch window: Directory and FAT with all entries equal to 255,
                        // because disk is empty
	testbuildbuff("file1-b1");
  OS_File_Append(fileA, Buff);       
	fileB=OS_File_New();
	testbuildbuff("file2-b1");
	OS_File_Append(fileB, Buff);       
	testbuildbuff("file2-b2");
  OS_File_Append(fileB, Buff);      
	testbuildbuff("file1-b2");
  OS_File_Append(fileA, Buff);      
	testbuildbuff("file1-b3");
  OS_File_Append(fileA, Buff);      
	fileC=OS_File_New();
	testbuildbuff("file3-b1");
  OS_File_Append(fileC, Buff);      	
	testbuildbuff("file1-b4");
  OS_File_Append(fileA, Buff);     
	testbuildbuff("file1-b5");
  OS_File_Append(fileA, Buff);      
	testbuildbuff("file3-b2");
  OS_File_Append(fileC, Buff);      
	testbuildbuff("file3-b3");
  OS_File_Append(fileC, Buff);      
	
	uint8_t readBuff[512];
	uint8_t result;
	result=OS_File_Read(fileA, 0, readBuff);
	result=OS_File_Read(fileB, 0, readBuff);
	result=OS_File_Read(fileC, 0, readBuff);

	result=OS_File_Read(fileA, 1, readBuff);
	result=OS_File_Read(fileB, 1, readBuff);
	result=OS_File_Read(fileC, 1, readBuff);
	
	
	result=OS_File_Read(fileA, 2, readBuff);
	result=OS_File_Read(fileB, 2, readBuff);
	result=OS_File_Read(fileC, 2, readBuff);

	
  return result;      // Put a breakpoint here and observe values from previous call.
...
//int main_final(void){
  ...
  if(BSP_Button1_Input() == 0){ // run TExaS if Button1 is pressed
    ...
    TExaS_Init(GRADER, 7370);   // initialize the Lab 5 grader
...
/*
	...
	uint8_t readBuff[512];
	uint8_t result;
	result=OS_File_Read(n, 2, readBuff);
	result=OS_File_Read(m, 2, readBuff);
	result=OS_File_Read(p, 2, readBuff);
	
	result=OS_File_Read(n, 3, readBuff);
	result=OS_File_Read(m, 3, readBuff);
	result=OS_File_Read(p, 3, readBuff);
	
	result=OS_File_Read(n, 4, readBuff);
	result=OS_File_Read(m, 4, readBuff);
	result=OS_File_Read(p, 4, readBuff);
*/
  while(1){
